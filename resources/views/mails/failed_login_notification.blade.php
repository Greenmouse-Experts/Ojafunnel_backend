@component('mail::layout')
    ...
    @slot('subcopy')
        ...
        <p class="mailbody">
            Someone try to access your {{ config('app.name') }} Account at {{date('F j, Y : h:i:s A',strtotime($date))}}.
        </p>
        <br>
        ...
        <br/>
        <p class="mailbody">
           If you did not log in to your {{ config('app.name') }} Account at the time detailed above, please call <a href="tel:{{ config('settings.customer_support.phone') }}"> {{ config('settings.customer_support.phone') }} </a>; or <a href="mailto:{{ config('settings.customer_support.email') }}">email {{ config('settings.customer_support.email') }} </a>immediately. <br/>
        </p>
